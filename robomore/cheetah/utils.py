...
def Eva_with_qpos_logging(model_path=None, run_steps=1, folder_name=None, video=False, rewardfunc_index=None, morphology_index=None):
    ...
    xml_path = os.path.join(current_dir, "GPTCheetah.xml")

    env = FitnessWrapper(gym.make("GPTCheetahEnv", xml_file=xml_path,
...
def make_env():
    ...
    def _init():
        ...
        env = gym.make("GPTCheetahEnv", xml_file = xml_path)
        return env
    return _init
...
def Eva(model_path=None, run_steps=100, folder_name=None, video = False, rewardfunc_index=None, morphology_index=None):
    ...
    current_dir = os.path.dirname(os.path.abspath(__file__))
    xml_path = os.path.join(current_dir,"GPTCheetah.xml")
    env = FitnessWrapper(gym.make("GPTCheetahEnv", xml_file = xml_path, render_mode="rgb_array", width=1280, height=720))
    fitness_scores = []
    rewards = []
    best_fitness = -np.inf
    best_actions = []  # 记录动作序列
    ...
    # 视频录制
    if video:
        video_env = FitnessWrapper(RecordVideo(gym.make("GPTCheetahEnv", xml_file = xml_path,  render_mode="rgb_array", width=1280, height=720), video_folder=f"{folder_name}", name_prefix=f"reward{rewardfunc_index}morphology{morphology_index}GPTCheetahEnv"))
