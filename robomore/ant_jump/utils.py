...
# 构造相对路径
        ...
        env = gym.make("GPTAntEnv", xml_file = xml_path, healthy_z_range = (0, 100.0))
        return env
    return _init
...
def Eva_with_qpos_logging(model_path=None, run_steps=1, folder_name=None, video=False, rewardfunc_index=None, morphology_index=None):
    ...
    env = FitnessWrapper(gym.make("GPTAntEnv", xml_file=xml_path, healthy_z_range=(0, 100.0),
...
render_mode="rgb_array", width=1280, height=720))
    ...
    # 视频录制
    if video:
        video_env = FitnessWrapper(RecordVideo(gym.make("GPTAntEnv", xml_file = xml_path, healthy_z_range = (0, 100.0), render_mode="rgb_array", width=1280, height=720), video_folder=f"{folder_name}", name_prefix=f"reward{rewardfunc_index}morphology{morphology_index}GPTAnt"))
...
class FitnessWrapper(gym.Wrapper):
    def __init__(self, env):
        ...
        self.max_z = None    # 记录最大高度

    def reset(self, **kwargs):
        obs, info = self.env.reset(**kwargs)
        self.x_start = self.env.unwrapped.data.qpos[0]
        self.max_z = self.env.unwrapped.data.qpos[2]  # 重置最大高度
        return obs, info
    ...
    def step(self, action):
        ...
        if z_position > self.max_z:
            self.max_z = z_position

        # 计算位移
        x_position = self.env.unwrapped.data.qpos[0]
        total_distance = x_position - self.x_start

        # 存储 fitness 和 max_z
        info["fitness"] = total_distance
        info["max_z"] = self.max_z  # 把最大跳跃高度也存进去
