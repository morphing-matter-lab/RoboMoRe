...
class MujocoDesignEnv():
    def __init__(self, env_name, num_rollouts, minimize=False):
        ...
        if env_name == 'GPTWalkerEnv':
            ...
            # self.ub = np.array([
            #     0.02,     # Torso radius
            #     0.02,     # Leg segment 1 x
            #     0.02,     # Leg segment 1 y
            #     0.02,     # Leg segment 2 x
            #     0.02,     # Leg segment 2 y
            #     0.02,     # Foot x
            #     0.02,     # Foot y
            #     0.02,    # Leg segment 1 radius
            #     0.02,    # Leg segment 2 radius
            #     0.02,    # Foot radius
            # ])
    ...
    def __call__(self, design_params):
        ...
        init_walker_parameter = np.array([1.45, 1.06, 0.6, 0.1, -0.13, 0.26, 0.05, 0.05, 0.04, 0.06])
        design_params += init_walker_parameter
        try:
            print(design_params)
            # Generate XML using ant_design function
            xml_string = walker_design(design_params)
            
            # Save to a temporary file first
            tmp_xml = f"/tmp/walker_design_{os.getpid()}.xml"
            with open(tmp_xml, "w") as f:
                f.write(xml_string)
            
            # Copy to standard name that Train/Eva expect
            import shutil
            shutil.copy(tmp_xml, "GPTWalker.xml")
            ...
            material = compute_walker_volume(design_params)
