...
from gp_model import train_gp, train_svgp, train_fvgp, train_spgp, train_NNGP
...
from functions.mjpc_fun import CostFun
from functions.mjpc_fun_ostrich import CostFunOstrich
...
parser.add_argument('--model', default='gp', type=str, help='GP model name')
parser.add_argument('--algo', default='bo', type=str, help='BO optimizer name')
parser.add_argument('--acqf', default='ei', type=str, help='BO acquisition function name')
...
parser.add_argument('--round', default=1, type=int, help='Round of optimization')
parser.add_argument('--init_num', default=1, type=int, help='Initial point number of each round')
parser.add_argument('--eval_num', default=3000, type=int, help='Evaluation number of each round')
parser.add_argument('--batch_size', default=1, type=int, help='Batch size of each iteration')
parser.add_argument('--auto', default=0, type=int, help='whether adaptivaly adjust the depth')

parser.add_argument('--task', default='Dance', type=str, help='Optimization task name')
...
if __name__ == '__main__':
    ...
    if args.task == 'Kick':
        objective = CostFun(task_path='../../build/mjpc/tasks/musculoskeletal/tracking3/task.xml', mjpc_path = '../../')
    elif args.task == 'Walk':
        objective = CostFun(task_path='../../build/mjpc/tasks/musculoskeletal/tracking2/task.xml', mjpc_path = '../../')
    elif args.task == 'Dance':
        objective = CostFun(task_path='../../build/mjpc/tasks/musculoskeletal/tracking5/task.xml', mjpc_path = '../../')
    elif args.task == 'Stand':
        objective = CostFun(task_path='../build/mjpc/tasks/musculoskeletal/walk3/task.xml', params_path='/home/mct/Desktop/Projects/mpc2/Approx_GP/pd_torque_params.txt',mjpc_path = '../')
    else:
        objective = CostFunOstrich(task_path='../../build/mjpc/tasks/musculoskeletal/walk_gym/task.xml', mjpc_path = '../../')
    ...
    gen_ini = True
    # if isinstance(objective, mujoco_gym_env.MujocoGymEnv) or isinstance(objective, lunar_land.Lunar):
    init_X = torch.tensor([50.0, 1.0, 100.0, 0.5]).to(dtype=dtype, device=device)
    init_X = normalize(init_X, bounds)
    timesteps = np.zeros(0)
    initial_X = torch.tensor(init_X).reshape(1, -1).to(dtype=dtype, device=device)
    initial_Y, qpos = eval_objective(initial_X)
    initial_Y = initial_Y.reshape(-1, 1)
    
    

    t0 = time.time()
    timesteps = np.hstack((timesteps, 0))
        # same initial points
        ...
        for rep in range(n_repeat):
            ...
            if not isinstance(objective, SyntheticTestFunction):
                ...
                if not os.path.exists(save_path):
                    os.mkdir(save_path)
                if not os.path.exists(f'{save_path}/qpos'):
                    os.mkdir(f'{save_path}/qpos')
                np.save(f'{save_path}/qpos/qpos_{len(train_X)}.npy', qpos)
            ...
            while len(train_X) < n_evals:
                ...
                if gp_model == 'gp' or len(train_X) < induce_size:
                    ...
                    print("norm_Y", norm_Y.shape, norm_Y)
                ...
                new_Y, qpos = eval_objective(new_X.to(device=device))
                new_Y = new_Y.reshape(-1, 1)
                timesteps = np.hstack((timesteps, time.time()-t0))
                ...
                if dim < 100:
                    np.savez(f'./{save_path}/{rep}.npz', x=train_X.detach().cpu().numpy(), 
                             ...
                             max_depth=np.array(all_max_depth), depth_record=all_depth_record.detach().cpu().numpy(), timesteps=timesteps)
                else:
