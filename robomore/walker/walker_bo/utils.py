...
import sys
import numpy as np
# from GPTWalker import GPTWalkerEnv
...
def Eva_with_qpos_logging(model_path=None, run_steps=1, folder_name=None, video=False, rewardfunc_index=None, morphology_index=None):
    ...
    env = FitnessWrapper(gym.make("GPTAntEnv", xml_file=xml_path, healthy_z_range=(0.3, 4.0),
...
# 构造相对路径
        ...
        env = gym.make("GPTWalkerEnv", xml_file = xml_path, healthy_z_range = (-10, 10))
        return env
    return _init
...
def Eva(model_path=None, run_steps=100, folder_name=None, video = False, rewardfunc_index=None, morphology_index=None):
    ...
    current_dir = os.path.dirname(os.path.abspath(__file__))
    xml_path = os.path.join(current_dir,"GPTWalker.xml")
    env = FitnessWrapper(gym.make("GPTWalkerEnv", xml_file = xml_path, healthy_z_range = (-10, 10),
render_mode="rgb_array", width=1280, height=720))
    fitness_scores = []
    rewards = []
    best_fitness = -np.inf
    ...
    # 视频录制
    if video:
        video_env = FitnessWrapper(RecordVideo(gym.make("GPTWalkerEnv", xml_file = xml_path, healthy_z_range = (-10.0, 10.0), render_mode="rgb_array", width=1280, height=720), video_folder=f"{folder_name}", name_prefix=f"reward{rewardfunc_index}morphology{morphology_index}GPTWalker"))
