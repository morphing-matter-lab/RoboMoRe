...
# 构造相对路径
        ...
        env = gym.make("GPTAntEnv", xml_file = xml_path, healthy_z_range = (-10, 10))
        return env
    return _init
...
render_mode="rgb_array", width=1280, height=720))
    ...
    # 视频录制
    if video:
        video_env = FitnessWrapper(RecordVideo(gym.make("GPTAntEnv", xml_file = xml_path, healthy_z_range = (-10.0, 10.0), render_mode="rgb_array", width=1280, height=720), video_folder=f"{folder_name}", name_prefix=f"reward{rewardfunc_index}morphology{morphology_index}GPTAnt"))
